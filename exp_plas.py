...
parser.add_argument('--save_name', type=str, default='plas_SAOT')
parser.add_argument('--data_path', type=str, default='./geo-FNO/plasticity/plas_N987_T20.mat')
...
def main():
    ...
    pos_train = pos_train.reshape(ntrain, s1, s2, -1)
    pos_test = pos_test.reshape(ntest, s1, s2, -1)
    x_train = x_train.reshape(ntrain, s1, s2, -1)
    x_test = x_test.reshape(ntest, s1, s2, -1)
    y_train = y_train.reshape(ntrain, s1, s2, Deformation, T)
    y_test = y_test.reshape(ntest, s1, s2, Deformation, T)
