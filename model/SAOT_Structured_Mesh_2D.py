...
from model.Spectral_Attention import Spectral_Attention_Structured_Mesh_2D
...
class Model(nn.Module):
    ...
    def forward(self, x, fx, T=None):  #x: B, H, W, 2; fx: B, H, W, 1;   T: B,1; 
       # print('x, fx: ', x.shape, fx.shape) #x, fx:  torch.Size([4, 85, 85, 2]) torch.Size([4, 85, 85, 1])
        ...
        if fx is not None:
            fx = F.pad(fx, [0,0, 0,self.pad_w, 0,self.pad_h])  
            
            fx = torch.reshape(fx, (fx.shape[0], -1, fx.shape[-1]))
            fx = torch.cat((x, fx), -1) 
